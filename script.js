...
function generateCV() {
  ...
  if (nameField == "") {
    let mand = document.createElement('div');
    mand.innerHTML = "*field is mandatory"
    mand.classList.add("mand");
    let nfield = document.querySelector('.name');
    nfield.appendChild(mand);
    mand.style.display="block";
    setTimeout(()=>{
       nfield.removeChild(mand);
    },2000)
  } else {
  ...
  document.getElementById("gmailT").innerHTML =
    document.getElementById("gmailField").value;
  //Github
  ...
  if(!file){
    ...
    img.appendChild(mand);
    mand.style.display = "block";
    setTimeout(()=>{
        img.removeChild(mand);
    },2000)
  }
